...
def generate_oil_summary(oil_data: dict) -> str:
    ...
    system_prompt = (
        ...
        "任务目标：根据提供的实时盘面数据与最新 24 小时海内外资讯，输出一份高信息密度、逻辑硬核、言简意赅的《原油化工每日早报 · 晨报版》。"
        "要求：\n"
        "1. 务必结合开篇给出的实时原油/天然气涨跌点位做归因分析，重点聚焦昨夜外盘原油收盘表现、美油裂解价差与今日国内商品期货开盘前瞻；\n"
...
请根据以上素材，撰写《原油化工每日早报（晨报版）》。严禁口水话，严禁输出任何外部链接，严格遵循以下视觉分层排版输出：

# 🛢️ 原油与化工每日早报（{beijing_now.strftime("%Y年%m月%d日")} 晨报版）

> 💡 **【10秒核心总结】**
> * 📌 **总论**：（用1-2句大白话，提炼昨夜至今最核心的地缘/供需宏观驱动与今日方向研判）
> * 📈 **盘面**：（用1句大白话概括外盘原油收盘、天然气与下游石化品的联动现状）
